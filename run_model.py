...
#     return [dtype(d).to(device) for d in np.load(file_name + '.npz', allow_pickle=True)]
...
def load_tensor(file_name, dtype, device):
    ...
    # 遍历所有数组
    for key in data.files:  # 使用 .files 获取所有键
        array = data[key]  # 加载对应数组
        try:
            # 检查数据类型并转换为张量
            tensor = dtype(array).to(device)
            tensors.append(tensor)
        except Exception as e:
            print(f"Skipping invalid data in key '{key}': {e}")
...
if __name__ == "__main__":
    ...
    """Output files."""
    file_MAEs = './Data/output/MAEs--' + setting + '.txt'
    file_model = './Data/output/' + setting
